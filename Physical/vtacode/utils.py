...
class MazeDataset(Dataset):
    def __init__(self, num, length, partition, path):
        self.partition = partition
        dataset_ori = np.load(path)
        dataset = dataset_ori[0:num]

        num_seqs = int(dataset.shape[0] * 0.8)
        if self.partition == 'train':
            self.state = dataset[:num_seqs].transpose(0, 3, 2, 1) / 255.
        else:
            self.state = dataset[num_seqs:].transpose(0, 3, 2, 1) / 255.
...
def full_dataloader(seq_size, init_size, batch_size, data_path, num, test_size=16):
    train_loader = MazeDataset(num, length=seq_size + init_size * 2, partition='train', path=data_path)
    test_loader = MazeDataset(num, length=seq_size + init_size * 2, partition='test', path=data_path)
