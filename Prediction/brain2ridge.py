...
parser.add_argument('--brain_train_path',
                    default='/home/kuroda/PredNet/brain_activity/cortex/'
...
x_data = pickle.load(open(args.brain_train_path, 'rb'), encoding='binary')[:]
y_data = []
...
print('Time train regression ::: {:.2f} sec'.format(
...
print('Time::: {:.2f} sec'.format(
        eval_train_data_end - training_end))

## Load val data for eval
print('Loading val data:::')
x_val_data = pickle.load(open(brain_test_path, 'rb'), encoding='binary')[:]
y_val_data = []
...
print('Time val data ::: {:.2f} sec'.format(
...
save_end = time.time()
print('saving : {:.2f} sec'.format(
        save_end - eval_val_data_end))
print('total time ::: {:.2f} sec'.format(save_end - begin))
