"""PredNet architecture for saving internal representations."""
import os
import numpy
import chainer
...
class PredNet(chainer.Chain):
        ...
        for nth in range(self.layers):
            if nth != 0:
                self.add_link('ConvA' + str(nth),
                              L.Convolution2D(channels[nth - 1] *2,
                                              channels[nth], 3, pad=1))
            
            self.add_link('ConvP' + str(nth),
                          L.Convolution2D(r_channels[nth],
                                          channels[nth], 3, pad=1))
            ...
            if nth == self.layers - 1:
                ...
                self.add_link('ConvLSTM' + str(nth),
                              ConvLSTM(self.sizes[nth][3], self.sizes[nth][2],
                                       (self.sizes[nth][1] * 2,
                                        r_channels[nth + 1]), r_channels[nth]))
    ...
    def __call__(self, x):
        self.num_called += 1
        frame_id = 345 + (self.num_called - 1) * 60
        for nth in range(self.layers):
