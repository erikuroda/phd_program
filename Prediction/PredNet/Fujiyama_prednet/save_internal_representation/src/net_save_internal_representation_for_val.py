"""PredNet architecture to save internal representations for validation data."""
import os
import numpy
import chainer
...
class EltFilter(chainer.Link):
    def __init__(self, width, height, channels,
                 batchSize = 1, wscale=1, bias=0, nobias=False,
    ...
    def __init__(self, width, height, in_channels, out_channels, batchSize = 1):
        ...
        for nth in range(len(self.in_channels)):
            self.add_link('x_i' + str(nth),
                          L.Convolution2D(self.in_channels[nth], out_channels,
                                          3, pad=1, nobias=True))
...
class PredNet(chainer.Chain):
    def __init__(self, width, height, channels, save_dir,
                 r_channels = None, batchSize = 1):
        super(PredNet, self).__init__()
        self.num_called = 0
        self.save_dir = save_dir
        dirs = ['vval001', 'vval002', 'vval003', 'vval004', 'vval005']
        for i in range(len(dirs)):
            if not os.path.exists(self.save_dir + dirs[i]):
                os.makedirs(self.save_dir + dirs[i])
        ...
        for nth in range(self.layers):
            if nth != 0:
                self.add_link('ConvA' + str(nth),
                              L.Convolution2D(channels[nth - 1] *2, channels[nth], 3, pad=1))
            
            self.add_link('ConvP' + str(nth),
                          L.Convolution2D(r_channels[nth], channels[nth], 3, pad=1))
    ...
    def __call__(self, x):
        self.num_called += 1
        frame_id = 165 + (self.num_called - 1) * 60
        for nth in range(self.layers):
        ...
        for nth in range(self.layers):
            if nth == 0:
                ...
                A = F.max_pooling_2d(F.relu(getattr(self, 'ConvA' + str(nth))(E[nth - 1])),
                                     2, stride = 2)
        ...
        numpy.savez_compressed(self.save_dir + dir + '%08d.npz' % (frame_id),
                               R0 = R[0].data, R1 = R[1].data, R2 = R[2].data, R3 = R[3].data)
        print('saved : '+ self.save_dir + dir + '%08d.npz' % (frame_id))
        return self.P0
