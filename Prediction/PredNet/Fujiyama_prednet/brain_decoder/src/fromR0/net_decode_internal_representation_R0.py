"""Network architecture definition for decoding predictive images given estimated R0."""
import numpy
import chainer
...
class PredNet(chainer.Chain):
    def __init__(self, width, height, channels, path,
    ...
    def __call__(self, x):
        ...
        # Inserting estimated R0 into PredNet means ignoring this commented out
        # fraction of code.
        '''
        for nth in range(self.layers):
        ...
        if frame_id <= 3705:
            ...
            dir = 'vval005/'
        rep = numpy.load(self.path + dir + '%08d.npz' % (frame_id))
        rep = rep["R0"].astype(numpy.float32)
        setattr(self, 'P0', F.clipped_relu(getattr(self, 'ConvP0')(rep), 1.0))
        return self.P0
