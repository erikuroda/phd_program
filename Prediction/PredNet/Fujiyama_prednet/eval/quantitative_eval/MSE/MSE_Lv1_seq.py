...
'''
This program is for comparison between (input, correct) and (predict, correct)
in a specific sequence.
If (input, correct) > (predict, correct), then the learned model can generate
better predictive frames rather than reconstructing the last input frames.

This calculates and outputs the average of MSE at (input, correct) and
(predict, correct).
(RGB value / 255)
...
for nth in range(begin_id, end_id):
    ...
    input_correct_loss += mean_squared_error(chainer.Variable(t_x),
...
mean_input_correct_MSE = input_correct_loss / num_pair
mean_predict_correct_MSE = predict_correct_loss / num_pair
end_time = time.time()

if check == num_pair:
    print("Successfully checked!!")
print('mean_input_correct_MSE: {}'.format(mean_input_correct_MSE))
print('mean_predict_correct_MSE: {}'.format(mean_predict_correct_MSE))

print('input_correct_loss: {}'.format(input_correct_loss))
print('predict_correct_loss: {}'.format(predict_correct_loss))
print('num_pair: {}'.format(num_pair))

if mean_input_correct_MSE > mean_predict_correct_MSE:
    print("The model can successfully generate better predictive frames rather than " +
          "reconstructing the last input frames!!")
else:
    print("The model generates worse pridictive frames rather than "+
          "reconstructing the last input frames...")
