...
parser.add_argument('--model_name', type=str,
                    help='Model name (recommended a name indentifying the depth and alphe).')
...
for i in range(0, 4497):
    ...
    internal_representation = np.load(
        internal_representation_path + dir + str(frame_id) + '.npz')
...
for i in range(0, 300):
    ...
    if frame_id <= 3705:
        ...
        dir = 'vval005/'
    internal_representation = np.load(internal_representation_path + dir + str(frame_id) + '.npz')
...
for i in range(predicted_y.shape[0]):
    ...
    print('saved : ' + predicted_save_path + depth + '/' + dir + '%08d.npz' % (frame_id))
